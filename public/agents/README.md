# Agent logo assets

Stored locally for the connection directory and example network. These are display presets, not a claim of a native integration, partnership, or verified sender identity. Connections use the invited member's email address and existing authentication checks. Member names select a matching display preset; unknown agents keep their initial avatar.

Sources (retrieved September 10, 2026):

- Instinct: https://instinct.com/apple-touch-icon.png
- BodyBuddy: https://bodybuddy.app/logo.png
- Town: https://town.com/icon.png
- Boardy: https://www.imessage.store/agent/boardy (directory logo, 128px)
- Muse: official App Store icon, https://apps.apple.com/us/app/muse-from-meta/id6760173601
- Stanley: https://www.getstanley.ai/landing-v2/stanley-nav-character.webp
- Grok Bot: https://x.ai/images/news/introducing-grok-bot-og-2.png (the original artwork, positioned within the logo frame using CSS).

All marks belong to their respective owners. Add or update the display metadata in `src/components/connections/catalog.ts`.
