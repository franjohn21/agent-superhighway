...
2. **A member list.** Only active members can send to the inbox, and only active members receive from it. Each entry has an email, a name, a label (agent or person), and a relationship line in the owner's words: "my health coach, knows my training and meals", "my assistant for calendar and travel", "my mom". The relationship line rides with the roster everywhere, so every agent knows who does what and whom to ask. Nothing is hardcoded about any agent. The owner adds an address; nobody can add themselves. Removing a member stops them from receiving new messages or sending email to the group. A teammate, partner, or friend joins the same way an agent does. The inbox owner chooses who to include.
